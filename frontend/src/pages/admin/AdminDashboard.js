...
const AdminDashboard = () => {
  ...
  // ✅ useCallback added (fix for useEffect dependency)
  ...
  // ✅ clean useEffect (no eslint error)
  useEffect(() => {
    fetchLeads();
  }, [fetchLeads]);
      ...
      <div className="max-w-7xl mx-auto px-5 md:px-8 py-10">
        ...
        <div className="glass rounded-2xl overflow-hidden">
          <div className="px-6 py-4 border-b border-white/5 flex items-center justify-between">
            ...
            <span className="text-xs text-slate-500 font-dm">
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b border-white/5">
                    {['Name', 'Email', 'Company', 'Service', 'Message', 'Date'].map((h) => (
                      <th
                        key={h}
                        className="text-left px-6 py-3 text-xs font-syne font-semibold text-slate-500 uppercase tracking-wider"
                      >
                        {h}
                      </th>
                    ))}
                ...
                <tbody>
                  {leads.map((lead, i) => (
                    ...
                    >
                      <td className="px-6 py-4 text-white text-sm font-dm">
                      ...
                      <td className="px-6 py-4">
                        <a
                          ...
                          className="text-sm font-dm text-electric-400 hover:text-electric-300"
                        >
                          {lead.email}
                        </a>
                      </td>

                      <td className="px-6 py-4 text-sm font-dm text-slate-400">
                        {lead.company || '—'}
                      </td>

                      <td className="px-6 py-4">
                        <span
                          className="inline-block px-3 py-1 rounded-lg text-xs font-dm capitalize"
                          style={{
                      ...
                      <td className="px-6 py-4 text-sm font-dm text-slate-400 line-clamp-1 max-w-xs">
                        {lead.message}
                      </td>

                      <td className="px-6 py-4 text-xs font-dm text-slate-500 whitespace-nowrap">
